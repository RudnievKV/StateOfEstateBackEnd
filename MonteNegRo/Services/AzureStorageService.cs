...
namespace MonteNegRo.Services
{
    public class AzureStorageService : IAzureStorageService
    {
        ...
        {
            _storageConnectionString = configuration.GetValue<string>("ConnectionStrings:AzureBlob");
        ...
        public async Task<BlobResponseDto> UploadAsync(IFormFile blob)
        {
            ...
            try
            {
                ...
                while (await container.GetBlobClient(blobName).ExistsAsync())
                {
                    blobName = Guid.NewGuid().ToString();
                }

                blobName = blobName + "." + blob.ContentType.Substring(blob.ContentType.IndexOf('/') + 1);
                // Get a reference to the blob just uploaded from the API in a container from configuration settings
                BlobClient client = container.GetBlobClient(blobName);

                var bytes = ConvertImageToByteArray(blob);
